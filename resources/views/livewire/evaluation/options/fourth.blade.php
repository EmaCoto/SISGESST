...
<div class="p-4">
    ...
    <div class="grid grid-cols-2">
        <select wire:model="selectedConsequence" class="cursor-pointer bg-gray-100 focus:text-black border focus:bg-white border-gray-300 rounded-lg mt-2 border-b border-t focus:border-t focus:border-b focus:border-0 shadow-lg focus:border-green-500 focus:ring-0 focus:outline-none">
            <option>Seleccione una opción</option>
            @foreach ($consequenceLevel as $consequence)
                <option value="{{ $consequence->value }}">{{ $consequence->name }}</option>
            @endforeach
        </select>
