    ...
    <div class="mt-10 w-1/5 rounded-3xl border-t-2 border-blue-500 shadow-lg shadow-slate-200 flex flex-col justify-center m-auto p-4">
        <form action="{{ route('process.update', $process) }}" method="POST" class="mt-1">
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="name" name="name" id="name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" value="{{ $process->name }}" required />
